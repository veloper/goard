...
// setupTestServer creates a real Goard server on a random port and
...
func TestCLIUsers(t *testing.T) {
	...
	createdUser, _ := created["user"].(map[string]any)
	if createdUser["username"] != "bot" {
		t.Errorf("expected username bot, got %v", createdUser["username"])
	}
